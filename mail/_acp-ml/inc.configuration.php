...
if ($_POST['frmAction'] == "save" ) {
  ...
  if ( trim($_POST['frmTrackOpens']) == 'ON' ) {
    $frmTrackOpens    = 'true';
  } else {
    $frmTrackOpens    = 'false';
  }
  ...
  // do the smtp
  $handle          = fopen("inc.settings_smtp.php", "r");
  if ($handle) { // settings
    while (!feof($handle)) {
    ...
    fclose($handle);
    if ($buildLines != '') {
      $handle          = fopen("inc.settings_smtp.php", "w+");
      if ($handle) {
        fwrite($handle, $buildLines);
        echo "updated: inc.settings_smtp.php<br />";
      } else {
        echo "inc.settings_smtp.php is not writable<br />";
      }
      fclose($handle);
    }
  }
  // do the db
  $buildLines      = '';
  $handle          = fopen("inc.settings_db.php", "r");
  if ($handle) { // settings
    while (!feof($handle)) {
      $line = fgets($handle, 4096);
      // Database
    ...
    fclose($handle);
    if ($buildLines != '') {
      $handle          = fopen("inc.settings_db.php", "w+");
      if ($handle) {
        fwrite($handle, $buildLines);
      } else {
        echo "inc.settings_db.php is not writable<br />";
      }
      fclose($handle);
    }
  }
  ...
  if ($handle) { // settings
    while (!feof($handle)) {
      ...
      $testStart = '$phpml[\'trackOpens\']';
      if (substr($line,0,strlen($testStart)) == $testStart) {
        $testStart = $testStart.str_repeat(" ", ($minLength - strlen($testStart)) );
        $line = $testStart . " = " . $frmTrackOpens . ";\n";
      }
      $buildLines .= $line;
    }
    fclose($handle);
    if ($buildLines != '') {
      ...
      if ($handle) {
        ...
        echo "inc.settings.php is not writable<br />";
      }
      fclose($handle);
    }
  }
  ...
  <div class="tpl_table">
    ...
    <table class="tpl_listing tpl_form" width="95%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="30%" style="text-align:right;"><?php echo $PHPML_LANG["trackopens"]; ?></td>
        <td style="text-align:left;"><input type="checkbox" name="frmTrackOpens" value="ON" <?php echo ($phpml['trackOpens'] === true) ? 'checked' : ''; ?>> <?php echo $PHPML_LANG["trackopens_explain"]; ?></td>
      </tr>
      <?php
      if ( DBHOST == '' || DBNAME == '' || DBUSER == '' || DBPASS == '' ) {
        ?>
        ...
        <tr>
          ...
          <td style="text-align:left;"><input name="frmDBpass" style="width:250px;" value="<?php echo DBPASS; ?>"></td>
        </tr>
        <?php
      }
      ?>
...
$pgArr['help']     = "When selecting SMTP the variables that apply to SMTP settings will display. For small to medium sizes mailing lists, Sendmail is a good choice.<br />";
ob_end_clean();
echo getSkin ($pgArr);
